(function($) {
  $(Screw).bind("before", function(){
    function exampleName(element){
      var contextName = '';
      var exampleName = $(element).children("h2").text();

      $(element)
        .parents('.describe')
        .children('h1')
        .each(function(index, element) {
          contextName = $(element).text() + ' ' + contextName;
      });

      return $.trim(contextName) + ' ' + exampleName;
    }
